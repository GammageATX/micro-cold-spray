...
@pytest.fixture
def mock_lock():
    """Create a mock lock for testing thread safety."""
    lock = MagicMock()
...
def test_double_check_locking(mock_lock):
...
def test_cleanup_with_lock(mock_lock):
...
def test_concurrent_initialization(mock_service):
...
@pytest.mark.parametrize("cleanup_count", [1, 2, 3, 5])
def test_cleanup_multiple_times(cleanup_count):
    """Test cleanup can be called any number of times safely."""
    # Create service
    singleton.get_config_service()
...
@pytest.mark.parametrize("thread_count", [1, 2, 5, 10])
def test_thread_safety_parametrized(thread_count):
    """Test thread safety with different numbers of threads."""
    instances = []
    
    def get_instance():
        instances.append(singleton.get_config_service())
    
    threads = [threading.Thread(target=get_instance) for _ in range(thread_count)]
    for t in threads:
        t.start()
    for t in threads:
        t.join()

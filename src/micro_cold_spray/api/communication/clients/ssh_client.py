...
class SSHClient:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"SSHClient initialized for {self._username}@{self._host}")
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if is_reachable:
                ...
                logger.warning(
                    f"Feeder controller at {
                        self._host} is not reachable")
                self._connected = False
    ...
    async def write_command(self, command: str) -> None:
        ...
        try:
            if not self._connected:
                if not await self.test_connection():
                    raise HardwareError(
                        "Cannot send command - feeder controller not connected", "feeder")

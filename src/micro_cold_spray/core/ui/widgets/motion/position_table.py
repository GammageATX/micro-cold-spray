...
class PositionTable(BaseWidget):
        ...
        try:
            ...
            if self._motion_tab:
                ...
                await self._ui_manager.send_update(
                    ...
                    {
                        "source": "position_table",
                        "message": "No motion tab reference available",
                        "level": "warning"
                    }
                )
    ...
    async def handle_ui_update(self, data: Dict[str, Any]) -> None:
        ...
        try:
            if "motion/position" in data:
                position_data = data["motion/position"]
                ...
                self._is_connected = data.get("connected", False)
                logger.debug(f"Connection state changed: {self._is_connected}")
    ...
    def _update_button_states(self) -> None:
        ...
        try:
            self._add_btn.setEnabled(True)  # Always allow adding positions
            self._remove_btn.setEnabled(True)  # Always allow removing positions
            self._move_btn.setEnabled(self._is_connected)  # Only allow moves when connected
        except Exception as e:

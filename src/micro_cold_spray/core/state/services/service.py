...
from micro_cold_spray.core.messaging import MessagingService
from micro_cold_spray.core.communication import CommunicationService
from micro_cold_spray.core.state.models import (
    StateCondition,
    StateConfig,
    StateTransition,
    StateRequest,
...
class StateService(ConfigurableService):
    ...
    async def _start(self) -> None:
        ...
        try:
            ...
            if not isinstance(config.data, dict):
                raise ConfigurationError("Invalid state configuration: not a dict")
            
            # Validate state config structure
            state_config = config.data
            if not isinstance(state_config, dict):
                raise ConfigurationError("Invalid state configuration structure")
                
            if "initial_state" not in state_config or "transitions" not in state_config:
                raise ConfigurationError("Invalid state configuration: missing required keys")
            ...
            raise ServiceError("Failed to start state service", {"error": str(e)})
    ...
    async def transition_to(self, request: StateRequest) -> StateResponse:
        ...
        if request.target_state not in self._state_machine:
            raise StateError(f"Invalid target state: {request.target_state}")
    ...
    async def get_conditions(self, state: Optional[str] = None) -> Dict[str, bool]:
        ...
        """
        if not self.is_running:
            raise ServiceError("Service not running")
        ...
        try:
            ...
            raise ServiceError(f"Failed to check conditions: {str(e)}")
    ...
    async def _check_condition(self, condition: StateCondition) -> bool:
        ...
        try:
            ...
            raise ServiceError(f"Failed to check condition: {str(e)}")

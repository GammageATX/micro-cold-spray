...
from micro_cold_spray.core.errors.exceptions import ValidationError
from micro_cold_spray.core.config.models import ConfigValidationResult
...
class RegistryService(BaseService):
    ...
    async def _start(self) -> None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Failed to start registry service: {str(e)}"
            )
                ...
                for key, value in data.items():
                    if key == ref_key and isinstance(value, str):
                        if not exists_check(value):
                            errors.append(
                                f"{path}: Unknown {ref_type} reference '{value}'"
                            )
                    else:
                        self._validate_reference(
                            value, f"{path}.{key}",
    ...
    def _tag_exists(self, tag: str) -> bool:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to check tag existence: {str(e)}"
            )
    ...
    def _action_exists(self, action: str) -> bool:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to check action existence: {str(e)}"
            )
    ...
    def _validation_exists(self, validation: str) -> bool:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to check validation existence: {str(e)}"
            )
    ...
    async def _load_tag_registry(self) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to load tag registry: {str(e)}"
            )
    ...
    async def _load_action_registry(self) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to load action registry: {str(e)}"
            )
    ...
    async def _load_validation_registry(self) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to load validation registry: {str(e)}"
            )
    ...
    async def update_tag_references(
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to update tag references: {str(e)}"
            )

...
from micro_cold_spray.core.config.models import FormatMetadata
    ...
    async def _start(self) -> None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Failed to start format service: {str(e)}"
            )

    async def _check_health(self) -> Dict[str, Any]:
        """Check format service health."""
        return {
            "validators": len(self._format_validators),
    ...
    def register_format(
        ...
        if format_type in self._format_validators:
            raise HTTPException(
                ...
                detail=f"Format {format_type} already registered"
            )
        ...
        try:
            ...
            logger.error(f"Failed to register format {format_type}: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to register format: {str(e)}"
            )

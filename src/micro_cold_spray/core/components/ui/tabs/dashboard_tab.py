...
class DashboardTab(BaseWidget):
    ...
    async def handle_ui_update(self, data: dict) -> None:
        ...
        try:
            if "system.state" in data:
                ...
                logger.debug(f"Dashboard received system state update: {state}")
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            if hasattr(self, '_sequence_control') and self._sequence_control is not None:
                if hasattr(self._sequence_control, 'cleanup') and callable(self._sequence_control.cleanup):
                    await self._sequence_control.cleanup()
                    
            if hasattr(self, '_progress_display') and self._progress_display is not None:
                if hasattr(self._progress_display, 'cleanup') and callable(self._progress_display.cleanup):
                    await self._progress_display.cleanup()
                    
            if hasattr(self, '_data_widget') and self._data_widget is not None:
                if hasattr(self._data_widget, 'cleanup') and callable(self._data_widget.cleanup):
